...
JFlow 的核心思想来源于浏览器渲染 HTML 的布局方式，数据和最终呈现的点线图间存在一层不同于传统HTML的布局，这层布局可以由数据计算生成，这层交由框架使用者具体问题具体实现，因此与JFlow对接的不是具体数据，而是由数据生成的布局节点和布局方法，提高了JFlow的可拓展性。另外单个数据对应的展示形式又回到了传统 HTML 的渲染模式，JFlow内置了一部分类型 flex的布局和事件系统，方便节点内部实现样式，传递事件。
...
```
基于这个数据结构，我们需要基于接口 [Layout]({@link Layout}) 实现一套对应的布局节点和一个布局类。
布局类描述了内部节点的排布和连线的信息。
...
class DemoLayout {
    ...
    reflow(jflow) {
        ...
        nodes.forEach((node, idx) => {
            // 计算 节点位置
            // getRenderNodeBySource 可以通过节点的原始数据找到对应的绘图单元
